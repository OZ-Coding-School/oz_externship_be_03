...
def _normalize_sparse_rows_l1(matrix: MatrixLike, return_format: str = "coo") -> MatrixLike:
    """
    희소 행렬의 각 행을 L1 정규화 (각 행의 합 = 1)

    목적: 사용자별 활동량 차이를 제거하여 공정한 비교 가능
    예: 북마크 100개 사용자와 10개 사용자를 동일 스케일로 정규화
...
class DataLoader:
    """
    ALS 추천 시스템을 위한 데이터 로더

    역할:
    1. 사용자-강의 상호작용 데이터 수집 (북마크, 검색, 스터디)
    2. 시간 감쇠 적용 (최근 활동에 더 높은 가중치)
    3. 아이템 피처 추가 (카테고리 매칭, 평점)
    4. 희소 행렬 구축 및 정규화
    5. 증분 학습(Partial Fit) 지원

    캐싱 전략:
    - 강의-카테고리 맵: 메모리 → Redis/Django 캐시 → DB
    - TTL 기반 자동 갱신으로 최신 데이터 유지
    ...
    def _get_decay_factor(self, created_at: datetime, reference_time: datetime) -> float:
        ...
        return float(0.5 ** (days_since / HALF_LIFE_DAYS))  # 지수 감쇠 적용

    def _get_decay_factor_array(self, days_since: np.ndarray) -> np.ndarray:
        """
        벡터화된 시간 감쇠 계산 (대량 데이터 처리용)

        목적: 루프 대신 NumPy 벡터 연산으로 수백~수천 배 성능 향상
    ...
    def get_lecture_category_map(self) -> Dict[int, Set[int]]:
        ...
        try:
            ...
            # Redis/Django 캐시에 저장 (Set → List 변환, JSON 직렬화 호환)
    ...
    def _load_user_interactions(
        ...
        Args:
            last_trained_at: 마지막 학습 시각 (Partial Fit 시 이후 데이터만 조회)
        ...
        """
        # 상호작용 점수 저장 (defaultdict로 자동 초기화)
        ...
        if weighted_study > 0:
            try:
                ...
                # 스터디 강의 데이터 조회 (그룹 ID, 강의 ID, 생성 시각)
    ...
    def _load_search_interactions(
        ...
        try:
            # 검색 로그 조회 기간 설정 (기간 제한 적용, 최근 30일)
    ...
    def build_user_item_matrix(
        ...
        3. 상호작용 피처 로드 (북마크, 검색, 스터디, 카테고리, 평점)
        4. 모든 피처 점수 병합
        5. 희소 행렬 구축 및 정규화
        ...
        if is_partial_fit:
            ...
            if last_trained_at:
                ...
                # 조기 검증: 신규 사용자 확인 (행렬 구축 전 수행)
                # 목적: 신규 사용자 발견 시 즉시 반환하여 불필요한 연산 방지
                ...
                if new_users_in_interactions:
                    ...
                    # 신규 사용자 발견 → Full Training 트리거를 위해 즉시 반환
                    # 빈 행렬과 is_new_user_added=True 플래그 반환
                    return (
        ...
        # 5.1 강의 ID 리스트 결정 (행렬의 Column)
        if not is_partial_fit:
            # Full Training: 모든 강의 ID 포함 (Cold Start 대비)
            # 신규 강의도 추천 가능하도록 전체 강의 목록 로드
        ...
        # 10. 결과 반환
        # (행렬, user_to_idx, lecture_to_idx, users, lectures, is_new_user_added)
