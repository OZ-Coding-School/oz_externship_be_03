...
def make_permission_patches(email_sub: Optional[str], phone_sub: Optional[str]) -> Tuple[Any, Any]:
    ...
    p1 = patch.object(P.EmailVerifiedPermission, "has_permission", new=_email_has_perm)
    p2 = patch.object(P.PhoneVerifiedPermission, "has_permission", new=_phone_has_perm)
    return p1, p2
...
class SignupViewTests(IsolatedRedisTestClient):
    ...
    def test_signup_success_201(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(UserSignupView, "SERVICE_CLASS", new=FakeSignupServiceOK))
    ...
    def test_serializer_invalid_400(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            assert body.get("error") == "입력값을 확인해주세요."
    ...
    def test_email_claim_mismatch_400(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            assert resp.status_code == status.HTTP_400_BAD_REQUEST
            assert body.get("error") == "인증 이메일과 요청 이메일이 일치하지 않습니다."
            assert "errors" in body and "email" in body["errors"]
    ...
    def test_phone_claim_mismatch_400(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            error_text: str = str(body.get("error", ""))
            assert "휴대폰" in error_text or "phone" in error_text

            assert "errors" in body and isinstance(body["errors"], dict)
            assert "phone_number" in body["errors"]

            msgs = body["errors"]["phone_number"]
            assert isinstance(msgs, list) and len(msgs) > 0
            assert any(("불일치" in m) or ("인증" in m) for m in msgs)
    ...
    def test_service_raises_drf_validation_400(self) -> None:
        ...
        with ExitStack() as stack:
            stack.enter_context(p1)
            stack.enter_context(p2)
            stack.enter_context(patch.object(UserSignupView, "SERVICE_CLASS", new=FakeSignupServiceRaisesDRF400))
    ...
    def test_service_raises_422(self) -> None:
        ...
        with ExitStack() as stack:
            stack.enter_context(p1)
            stack.enter_context(p2)
            stack.enter_context(patch.object(UserSignupView, "SERVICE_CLASS", new=FakeSignupServiceRaises422))

            resp: Response = self.client.post(self.url, data=valid_payload(), format="json")
            body: Dict[str, Any] = cast(Dict[str, Any], resp.data)

            assert resp.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
            assert body.get("error") == "인증 절차가 완료되지 않았습니다."
    ...
    def test_service_raises_409(self) -> None:
        ...
        with ExitStack() as stack:
            stack.enter_context(p1)
            stack.enter_context(p2)
            stack.enter_context(patch.object(UserSignupView, "SERVICE_CLASS", new=FakeSignupServiceRaises409))

            resp: Response = self.client.post(self.url, data=valid_payload(), format="json")
            body: Dict[str, Any] = cast(Dict[str, Any], resp.data)

            assert resp.status_code == status.HTTP_409_CONFLICT
            assert body.get("error") == "이미 사용 중인 입력값이 있습니다."
            assert "errors" in body and "email" in body["errors"]
    ...
    def test_service_raises_500(self) -> None:
        ...
        with ExitStack() as stack:
            stack.enter_context(p1)
            stack.enter_context(p2)
            stack.enter_context(patch.object(UserSignupView, "SERVICE_CLASS", new=FakeSignupServiceRaises500))

            resp: Response = self.client.post(self.url, data=valid_payload(), format="json")
            body: Dict[str, Any] = cast(Dict[str, Any], resp.data)

            assert resp.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
            assert body.get("error") == "요청 처리 중 오류가 발생했습니다."
            assert "errors" in body
...
class SignupServiceTests(IsolatedRedisTestClient):
    ...
    def test_role_flag_roundtrip(self) -> None:
        assert role_from_flags(is_staff=False, is_superuser=False) == Role.USER.value
        assert role_from_flags(is_staff=True, is_superuser=False) == Role.STAFF.value
        assert role_from_flags(is_staff=True, is_superuser=True) == Role.ADMIN.value

        assert flags_from_role(Role.USER.value) == {"is_staff": False, "is_superuser": False}
        assert flags_from_role(Role.STAFF.value) == {"is_staff": True, "is_superuser": False}
        assert flags_from_role(Role.ADMIN.value) == {"is_staff": True, "is_superuser": True}

        assert status_from_active(True) == "active"
        assert status_from_active(False) == "inactive"
        assert active_from_status("active") is True
        assert active_from_status("inactive") is False

    def test_sign_up_success_user_role(self) -> None:
        svc = DefaultSignupService()

        with (
            patch("apps.users.services.user_signup_service.User.objects.create_user") as create_user,
            patch("apps.users.services.user_signup_service.validate_nickname") as v_nick,
            patch("apps.users.services.user_signup_service.validate_name") as v_name,
            patch("apps.users.services.user_signup_service.validate_korean_phone") as v_phone,
            patch("apps.users.services.user_signup_service.validate_birthday") as v_birth,
        ):
    ...
    def test_sign_up_success_staff_admin_role_and_str(self) -> None:
        ...
        with (
            patch("apps.users.services.user_signup_service.User.objects.create_user") as create_user,
            patch("apps.users.services.user_signup_service.validate_nickname"),
            patch("apps.users.services.user_signup_service.validate_name"),
            patch("apps.users.services.user_signup_service.validate_korean_phone"),
            patch("apps.users.services.user_signup_service.validate_birthday"),
        ):
            ...
            assert kw_staff["is_staff"] is True and kw_staff["is_superuser"] is False
            ...
            assert kw_admin["is_staff"] is True and kw_admin["is_superuser"] is True
    ...
    def test_sign_up_validation_aggregates_field_errors(self) -> None:
        ...
        with (
            ...
            assert any("닉네임" in str(x) for x in d["nickname"])
            assert any("이름" in str(x) for x in d["name"])
            assert any("휴대폰" in str(x) for x in d["phone_number"])
            assert any("생년월일" in str(x) for x in d["birthday"])
    ...
    def test__extract_msgs_with_detail_dict_mixed(self) -> None:
        ...
        assert len(msgs) == 3
        assert "이미 사용 중입니다." in msgs
        assert "형식이 올바르지 않습니다." in msgs
        assert "필수 입력값입니다." in msgs

    def test__extract_msgs_with_detail_dict_all_lists(self) -> None:
        # detail이 dict 형태이며 모든 값이 list
        exc = DRFValidationError(
            detail={
    ...
    def test__extract_msgs_message_dict(self) -> None:
        ...
        msgs = _extract_msgs(exc)
        assert len(msgs) == 3
        assert "msg1" in msgs and "msg2" in msgs and "msg3" in msgs
