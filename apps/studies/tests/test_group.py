...
class MemberLeaveAPITestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.client.force_authenticate(user=self.user)
        self.group_id = "00000000-0000-0000-0000-000000000007"
        self.url = reverse("study-member-leave", kwargs={"group_id": self.group_id})

    def test_leave_group_cases(self) -> None:
        """자진 탈퇴 API: 인증 상태별 응답 코드 검증"""
        cases = [
            {
                "label": "로그인된 사용자가 탈퇴 요청",
                "auth": True,
                "data": {"member_id": 1},
                "expected": status.HTTP_200_OK,
            },
            {
                "label": "비로그인 사용자가 탈퇴 요청",
                "auth": False,
                "data": {"member_id": 1},
                "expected": status.HTTP_401_UNAUTHORIZED,
            },
        ...
        for case in cases:
            with self.subTest(msg=case["label"]):
                if not case["auth"]:
                    self.client.logout()
                response = self.client.delete(self.url, data=case["data"], format="json")
                self.assertEqual(response.status_code, case["expected"])
