...
class StudyGroupCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_max_headcount(self, value):
        if not 2 <= value <= 10:
            raise serializers.ValidationError("최대 인원 수는 2명 이상 10명 이하로 설정해야 합니다.")
        return value

    def validate_lectures(self, value):
        if len(value) > 5:
    ...
    def validate(self, attrs):
        ...
        if start and start < today:
            raise serializers.ValidationError({"start_at": "스터디 시작일은 오늘 이후여야 합니다."})
        if start and end and end < start + timedelta(days=5):
            raise serializers.ValidationError({"end_at": "스터디 종료일은 시작일보다 최소 5일 이후여야 합니다."})
        return attrs
...
class StudyLectureSerializer(serializers.ModelSerializer):
...
class AdminStudyGroupListSerializer(serializers.ModelSerializer):
    current_members = serializers.SerializerMethodField()
...
class AdminStudyGroupDetailSerializer(serializers.ModelSerializer):
    members = GroupMemberSerializer(many=True, read_only=True)
    lectures = StudyLectureSerializer(many=True, read_only=True)
    current_members = serializers.SerializerMethodField()
