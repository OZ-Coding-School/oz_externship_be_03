...
class NotificationTaskTests(TestCase):
    ...
    def testk_task_execution_and_result(self) -> None:

        result = create_recruitment_applicant_task.apply(args=[1, self.user.id])

        self.assertTrue(result.successful())
        self.assertEqual(result.state, "SUCCESS")

        self.assertIn("notification_id", result.result)
        self.assertIsInstance(result.result["notification_id"], int)
