...
def get_user_notifications(user_id:int):
    '''Redis에서 새 알림 확인 -> DB에서 상세 정보 가져오기'''
    ...
    last_processed_time = cache.get(last_processed_key, timezone.now() - timedelta(minutes=1))
    ...
    if new_notifications.exists():
        cache.set(last_processed_key, timezone.now(), timeout=86400) # 24시간 캐시
