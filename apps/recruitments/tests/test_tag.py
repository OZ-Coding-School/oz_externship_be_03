...
class TestTagMockView(TestCase):
    ...
    def test_get_tags(self) -> None:
        response: Any = self.client.get("/api/v1/recruitment-tags/")
    ...
    def test_post_tag_success(self) -> None:
        ...
        response: Any = self.client.post("/api/v1/recruitment-tags/", data)
    ...
    def test_post_tag_missing_name(self) -> None:
        response: Any = self.client.post("/api/v1/recruitment-tags/", {})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("태그 이름은 필수", str(response.data))

    # 중복된 태그 → 400
    def test_post_duplicate_tag(self) -> None:
        response: Any = self.client.post("/api/v1/recruitment-tags/", {"name": "Python"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
